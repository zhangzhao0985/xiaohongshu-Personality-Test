# 正式上线指南（海外免备案 / 大陆备案 两种方式）

这份是「正式上线」的保姆级步骤。整套是标准 Node 服务，海外 / 香港 / 大陆服务器都能跑。

> ⚠️ 先说清楚：买服务器、买域名、（如走大陆）ICP 备案、域名解析这几步都要用你的实名认证和账号，我（AI）无法代办。下面把每一步写清楚，你照着做或交给技术朋友即可。

---

## 能不能不备案？两种方式先二选一

| | 方式 A：海外 / 香港服务器（**免备案**） | 方式 B：大陆服务器（要备案） |
|---|---|---|
| 要不要 ICP 备案 | ❌ 不用 | ✅ 必须，且首页要显示备案号 |
| 多久能上线 | 当天就能上 | 备案审核约 1~3 周 |
| 大陆访问速度 | 香港最接近、较快；美国/欧洲较慢 | 最稳最快 |
| 适合 | 想尽快上线的个人小项目（**你看到的多数测试站就是这种**） | 长期、量大、追求最稳 |

- **想免备案** → 选 **方式 A**：服务器买在**香港**（离大陆最近、速度最好），把 `app.js` 的 `CONFIG.ICP_BEIAN` **留空**（首页底部就不会显示备案号）。其余步骤和下面一样，只是**跳过第 2 步备案**。
- 你说“看到别的测试站没有工信部备案”——它们基本都是**方式 A（挂在海外/香港）**，这是合法常见做法。⚠️ 注意：**只要服务器在大陆，就必须备案**，绕不过去；免备案的前提是服务器在境外。
- 哪里买香港服务器：阿里云国际 / 腾讯云国际的**香港轻量服务器**，或 Vultr / 搬瓦工的香港节点；也可用 Render / Railway（在美国，能用但大陆访问偏慢）。

> 下面第 1、3~9 步两种方式都适用；**只有第 2 步「备案」是方式 B 才需要**。方式 A 跳过第 2 步、并把 `ICP_BEIAN` 留空即可。
...
## 2. 买域名 + ICP 备案（**仅方式 B 需要**；最耗时，越早开始越好）
...
# 设置口令：复制 .env.example 为 .env，编辑里面的 ADMIN_TOKEN / TOKEN_SECRET
cp .env.example .env
nano .env

# 先手动试跑（临时在安全组放行 8787）。server.js 会自动读取 .env
node server.js
